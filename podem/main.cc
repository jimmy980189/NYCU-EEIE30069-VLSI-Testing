...
int SetupOption(int argc, char ** argv)
{
    ...
    option.enroll("path", GetLongOpt::NoValue,
            "print all possible paths connecting the given start (PI) and end (PO)", 0);
...
int main(int argc, char ** argv)
{
    ...
    if (option.retrieve("logicsim")) {
        ...
        //modified logic simulator for VLSI Testing - Assignment2
        Circuit.InitPattern(option.retrieve("input"));
        Circuit.MLogicSimVectors();
    }
        ...
        if (option.retrieve("start") && option.retrieve("end")) {
            Circuit.PrintAllPaths_Stack(option.retrieve("start"), option.retrieve("end"));
        }
        else {
            cout << "Missing input [start PI] or [end PO] for \"-path\"" << endl;
        }
    }
    else if (option.retrieve("pattern")) {
        if (option.retrieve("num")) {
            if (option.retrieve("unknown")) {
                Circuit.GenerateRandPattern(option.retrieve("output"), 
                        atoi(option.retrieve("num")), true);
            }
            else {
                Circuit.GenerateRandPattern(option.retrieve("output"), 
                        atoi(option.retrieve("num")), false);
            }
        }
        else {
